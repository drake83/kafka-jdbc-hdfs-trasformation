...
public class PriorityQueueSerde<T> implements Serde<PriorityQueue<T>> {
  ...
  public PriorityQueueSerde(final Comparator<T> comparator, final Serde<T> valueSerde) {
    inner = Serdes.serdeFrom(new PriorityQueueSerializer<T>(comparator, valueSerde.serializer()),
                             new PriorityQueueDeserializer<T>(comparator, valueSerde.deserializer()));
  }
